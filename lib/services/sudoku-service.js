...
async function loadSudokuModule() {
  ...
  try {
    if (typeof window === 'undefined') throw new Error('WASM modules can only be loaded in the browser');
...
export const SudokuService = {
  ...
  solvePuzzle: async (puzzle) => {
    try {
      ...
      // Solve the puzzle
      const result = sudokuEngine.solve(flatPuzzle);
  ...
  checkValidity: async (puzzle) => {
    try {
      ...
      const result = sudokuEngine.checkValidity(flatPuzzle);
  ...
  getHint: async (puzzle, row, col) => {
    try {
      ...
      // Get hint for the specified cell
      const result = sudokuEngine.getHint(flatPuzzle, row, col);
