...
async function loadRubixModule() {
  ...
  try {
    ...
    const module = await import(/* webpackIgnore: true */ '/wasm/rubix.js');
    rubixModule = await module.RubixModule();
    return rubixModule;
  } catch (error) {
...
export const RubixService = {
  ...
  initSolver: async () => {
    try {
      ...
      rubixSolver = new module.RubixCubeSolver();
      return { success: true };
    } catch (error) {
  ...
  solveCube: async (cubeState) => {
    try {
      ...
      const result = rubixSolver.solve(flatState);
