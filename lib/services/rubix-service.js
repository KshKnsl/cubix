...
export const RubixService = {
  ...
  solvePuzzle: async (startState, finalState) => {
    try {
      ...
      // TODO: The current C++ backend likely solves to a standard solved state.
      // The 'finalState' parameter is accepted here to match the frontend call,
      // but it's not used by rubixSolver.solve() without backend changes.
      const jsFlatState = startState.flat().flat().map(Number);

      const vecStartState = new rubixModule.IntVector();
      jsFlatState.forEach(n => vecStartState.push_back(n));

      rubixSolver.setStartState(vecStartState);
      vecStartState.delete(); // Delete after data is copied by setStartState
  ...
  isSolved: async (cubeState) => {
    try {
      ...
      return {
        solved: result === "true", // Assuming C++ returns "true" or "false" as string
      };
    } catch (error) {
  ...
  applyMove: async (cubeState, move) => {
    try {
      ...
      const result = rubixSolver.applyMove(vecCubeState, move);
      vecCubeState.delete();
  ...
  // Alias for initSolver, similar to other services
  compileSolver: async () => await RubixService.initSolver(),
};
