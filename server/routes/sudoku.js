...
router.post('/compile', async (req, res) => {
  try {
    ...
    const uniqueExe = path.join(tempDir, `sudoku_engine${exeExt}`);
...
router.post('/execute', async (req, res) => {
  try {
    ...
    const tempDir = path.join(os.tmpdir(), 'cubix-sudoku');
    const executablePath = path.join(tempDir, `sudoku_engine${os.platform() === 'win32' ? '.exe' : ''}`);
